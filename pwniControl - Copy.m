...
setappdata(handles.pwniControl,'currentlyAcquiring',false)
setappdata(handles.pwniControl,'currentlyAcquiring',false)
...
function acqTimerFunction(hObject,eventdata,hfigure)
% This function keeps an eye on background acqusition, saving files when
% needed, etc.
handles = guidata(hfigure);

% Handle the state where data is being acquired
...
if getappdata(handles.pwniControl,'acqState')
    setappdata(handles.pwniControl,'acqState',false)
    set(handles.startAcqBt,'ForegroundColor',[0 0 0])
    ...
    set(handles.realtimeBtn,'ForegroundColor',[1 0 0])
